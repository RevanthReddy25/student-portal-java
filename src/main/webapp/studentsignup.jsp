...
<%
    try {
        ...
        PreparedStatement ps = con.prepareStatement("SELECT pas FROM login WHERE rollno = ?");
        ps.setString(1, str1);
        ResultSet rs = ps.executeQuery();

        if (rs.next()) {
            String passwordFromDB = rs.getString("pas");
            ...
            if (str2.equals(passwordFromDB)) {
                ...
                if (str3.equals(str4)) {
                    ...
                    PreparedStatement updatePs = con.prepareStatement("UPDATE login SET pas = ? WHERE rollno = ?");
                    ...
                    out.println("window.location.href='studentsignup.html';");
                    out.println("</script>");
                }
            } else {
                out.println("<script>");
